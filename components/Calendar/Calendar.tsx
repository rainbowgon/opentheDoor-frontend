...
const Calendar = () => {
  ...
  return (
    <View>
      ...
      <TimeListView>
        {
          themeDetail?.timeSlotList && themeDetail?.timeSlotList.map(timeInfo => (
            <>
              {
                timeInfo.date && onCompareDate(timeInfo.date) &&
                timeInfo.timeList?.map((time) => (
                  <CustomButton mode={onModeSelected(time.isAvailable)} value={time.time} />
                ))
              }
            </>
          ))
